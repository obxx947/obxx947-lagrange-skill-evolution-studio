...
(async () => {
  ...
  for (let k = 0; k < 25; k++) {
    t3 = await ev(() => ({
      name: (document.getElementById('shipName') || {}).innerText || '',
      kv: (document.getElementById('shipKv') || {}).innerText || '',
      ctx: (document.getElementById('ctxBox') || {}).innerText || ''
    }));
    if (t3.name && t3.name.indexOf('加载中') < 0) break;
    await sleep(300);
  }
  check('3e 加点页显示舰名（异步加载完成）', t3.name.indexOf('乌拉诺斯之矛') >= 0, t3.name.split('\n')[0]);
  check('3f 加点页读到了舰船库数据', /结构值/.test(t3.kv) && /18\.9万|189493/.test(t3.kv), t3.kv.replace(/\s+/g, ' ').slice(0, 90));
  check('3g 加点页列出传入参数', t3.ctx.indexOf('uranus-spear') >= 0);
  check('3h 加点页点明「本轮只做入口」', (await ev(() => document.querySelector('.card h3').innerText)).indexOf('下一轮') >= 0);
