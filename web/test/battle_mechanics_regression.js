...
(async()=>{
  ...
  const C=await p.evaluate(()=>{
    /* ⚠️ hitMin/hitMax 设 100 不再是"必中"：2026-09-24 起引擎按资料把命中上限钳到 95%，
       所以 300 发里本来就有约 5% 是【未命中】，不能算进"被拦截"。
       判定改为：未掉血的次数必须集中在未命中那一档（≤10%），若拦截真生效会是 100%。 */
    ...
    const run=(wtype)=>{
      ...
      for(let k=0;k<300;k++){
        ...
        if(tgt.hp>=before) intercepted++;   // 没掉血 = 未命中(≤5%) 或 被拦下(直射应恒为0)
      }
      return intercepted;
    };
    return {direct:run('direct'), projectile:run('projectile')};
  });
  check('③ 直射武器不被拦截（300发里未掉血的只应是 5% 命中上限造成的未命中，≤10%）', C.direct<=30, '直射未掉血='+C.direct+'/300（拦下应为 0，其余是 95% 命中上限的 miss）');
