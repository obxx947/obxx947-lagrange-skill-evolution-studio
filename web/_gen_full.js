...
H(2, '1.1 ★★ 同位置节点 = 真·二选一（' + altGroups.length + ' 组，其中策略类 ' + stratAlt.length + ' 组）');
T += '**这是什么**：同一系统里，两个节点的**位置完全相同**（同一行同一列）且都不是空槽 —— 游戏里**只能点其中一个**。\n\n';
T += '**现状（严重）**：我的加点页**没做互斥**，玩家可以把两个都点上，引擎也会把两个的效果都算进去 —— 游戏里不可能同时拥有。\n\n';
stratAlt.forEach(g => {
