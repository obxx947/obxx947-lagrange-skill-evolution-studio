...
/* 代号全集（供 3.1 标题与"全部代号"一行用） */
const codeSet = new Set();
pureRefs.forEach(x => codeSet.add(x.token));
embedRefs.forEach(x => x.toks.forEach(t => codeSet.add(t)));
...
T += '| **★★** | **只看这一节 —— 需要你【填】/【定】/【我拿不准】的全部事项** | K1 19 项 · K2 14 项 · K3 12 项 |\n';
...
const _allW = [];
ships.forEach(s => {
  const w = a => { for (const x of (a || [])) _allW.push([s, x]); };
  w(s.weapons);
  Object.values(s.modules || {}).forEach(m => { if (m.weapons) w(m.weapons); Object.values(m.variants || {}).forEach(v => w(v.weapons)); });
});
const _n = f => _allW.filter(a => f(a[1])).length;
const _repair = _n(w => w.dpm && w.dpm.repair);
const _sysTgt = _n(w => w.subSystemTargets);
const _xN = _n(w => /[（(]\s*[×xX]\s*\d+\s*[)）]/.test(w.name || ''));
let _icpt = 0;
ships.forEach(s => Object.values(s.modules || {}).forEach(m => {
  const gs = m.variants ? Object.values(m.variants) : [m];
  gs.forEach(g => { if (g && g.interceptRate > 0) _icpt++; });
}));
const sev = ships.filter(s => s.evasion != null && s.evasion > 0).length;

H(1, '★★ 只看这一节 —— 需要你【填】/【定】/【我拿不准】的全部事项');
T += '> 这一节是**索引 + 穷举**。下面每一条都是"卡在你这儿"的，按三类分：\n';
T += '> **【填】**＝要你给数据 ｜ **【定】**＝要你拍口径 ｜ **【我拿不准】**＝我自己没把握、请你验。\n';
T += '> 更细的内容在下面各章节，这里只列**有什么、有几条、影响多大**。\n\n';

H(2, 'K1 【填】要你给数据（按影响从大到小）');
T += '| # | 事项 | 数量 | 影响 | 详见 |\n|---|---|---|---|---|\n';
T += '| 1 | **超主力反舰武器缺「大型舰船」命中行** | **' + noBigTarget.length + ' 门** | 打超主力时输出只有面板的约 1/7（浏览器实跑发现的） | §1.8 |\n';
T += '| 2 | **武器面板 DPM 算不出来** | **' + dpmBad.length + ' 门** | 单发 / 发数 / 冷却三者口径不一致，直接算错输出 | §1.3 |\n';
T += '| 3 | **武器名损坏**（名字是段落标题如「一、机载投弹系统（投射主武器系统）」） | **' + badW.length + ' 门** | 这些武器基本打不出伤害（单发 100、DPM 全 0） | §1.4 |\n';
T += '| 4 | **术语代号**（`{ED9140}` 这类，官网没公开术语表） | **' + pureRefs.length + ' 整条 + ' + embedRefs.length + ' 夹在中间** | 这些加点**完全读不出效果**，现在等于没生效 | §3.1 |\n';
T += '| 5 | **一条说明有多个 `{}` 数值位，对不上** | **' + multiFails.length + ' 个** | 数值位含义不明，加成无法汇总 | §3.2 |\n';
T += '| 6 | **系统破坏数据「找不到出处」** | **' + unsourced.length + ' 条** | 可能是我从资料里编的（其中 4 条互相雷同，疑复制错误） | §1.5 / §1.6 |\n';
T += '| 7 | **舰船缺「指挥系统」模块** | **' + noCmdSys.length + ' 艘** | 旗舰机制依赖指挥系统，缺了就算不了 | §1.7 |\n';
T += '| 8 | **武器名带 `(×N)` = 武器安装数** | **' + _xN + ' 门** | 引擎完全没乘这个数（见 K2-3） | §1.3 |\n';
T += '| 9 | **闪避率** | **' + sev + ' 艘有** | 命中公式里"敌方闪避"恒为 0，等于这个维度不存在 | §2 |\n';
T += '| 10 | **拦截率** | **只有 ' + _icpt + ' 处** | 其余舰船拦截恒为 0 | §2 |\n';
T += '| 11 | **系统攻击效率（低/中/高）** | **只有 ' + _sysTgt + ' 门有** | 剩下 ' + (412 - _sysTgt) + ' 门打不出系统伤害 | §2.3 |\n';
T += '| 12 | **系统「独立血量」** | **0 个系统有 hp 字段** | 现在引擎是"一发入魂"，不是"打满才毁"（见 K2-11） | §2.3 |\n';
T += '| 13 | **系统伤害系数**（资料测得 1.25 / 1.5 / 3） | **0 门有** | 全按 1.5 算（新大地该是 3、刺鳐 1.25） | §2.5 |\n';
T += '| 14 | **打结构系数**（资料：不全是 0.8，个别 <0.1） | **0 门有** | 打系统流白赚结构伤害 | §2.5 |\n';
T += '| 15 | **维修值** | **只有 ' + _repair + ' 门有** | 其余维修舰奶量为 0；且引擎无批次、无命中判定 | §2 |\n';
T += '| 16 | **载机作战模式 `flightMode`** | **' + noMode.length + ' 艘缺 / 共 ' + (noMode.length + withMode.length) + ' 艘载机** | 往复作战和独立作战分不出来 | §3 |\n';
T += '| 17 | **一门武器都没有** | **' + noW.length + ' 艘** | 全程零输出 | §2.6 |\n';
T += '| 18 | **连结构值都没有** | **' + noHp.length + ' 艘** | 会被兜底成 hp=1，一碰就碎 | §2.6 |\n';
T += '| 19 | **完全没有加点数据** | **' + noBp.length + ' 艘** | 加点页打不开 | §2.6 |\n\n';

H(2, 'K2 【定】要你拍板的口径（每条都写了两种可能 + 差多少）');
T += '| # | 事项 | 两种可能 | 差多少 | 我的倾向 / 现状 |\n|---|---|---|---|---|\n';
T += '| 1 | **调校系数** | 代码里写死 `1.3`（全库无条件 +30%）｜改成 1.0 或逐门给值 | **每门 30%** | 你自己的核对式 `(140-10+35)×2×1×4×60÷21=3771` 里**没有调校项** → 倾向改成 1.0 或逐门写值。**未动** |\n';
T += '| 2 | **离子炮 miss 口径** | 你说「每一次单独计算，未命中**该次**无效」（= 引擎现状）｜《星舰实验室 EP-37》原话「**没打中就结束当前攻击循环、直接进冷却**」 | **36%**（艾奥-攻坚·80% 命中：0.533 vs 0.339 发/秒） | 按你说的保留现状。但资料里那些实测数（南十字只打出面板 40%~58%）**全是按"断"反推的** |\n';
T += '| 3 | **武器名 `(×N)` 怎么处理** | 按名字硬乘 ｜ 用面板 DPM 逐门反推校准 | 2~6 倍 | 倾向**逐门反推**：你给的面板公式 5 门离子炮反推全是整数且与你口述吻合，而按名字硬乘这批面板本身不自洽（比值 0.16~3.4）。**未动** |\n';
T += '| 4 | **分伤的主/副承伤** | 引擎 `round(n/2.5)` + 每发随机挑（现状）｜资料：整数部分**主要承伤**、小数部分**副承伤**（掉血慢） | 6 艘时资料 2主1副、引擎给 2 | 倾向按资料改。**未动** |\n';
T += '| 5 | **攻击循环第一批的时机** | 引擎要 `batchTimer ≥ 持续/批数` → 落在 **4/8/12/16 秒**｜资料「从 **0 秒**开始，每隔 4 秒」→ **0/4/8/12 秒** | 整体晚一个间隔（空战先手影响大） | 倾向按资料改。全库 **157 门** atkDuration>0 受影响。**未动** |\n';
T += '| 6 | **护甲模型** | 版本A：固定数值线性减法（引擎现状）｜版本B：百分比减伤、上限 85%、比"护甲值 vs 武器穿透" | 换个武器就分开（500打300甲：两版都=200；1000打300甲：A=700、B=400） | **已定：用版本A**（你那份带数值的表全是 A 算出来的） |\n';
T += '| 7 | **能量单发「乘」** | 你第 1 条只写了「乘」｜但资料唯一有实测的两个数只有**加算**算得出来 | **40%** | 我按**加算**实现了（维塔斯B 552 ✓、星脉 64 ✓）。若你要纯乘算，一行改回 |\n';
T += '| 8 | **反拦截怎么算** | 引擎按**总拦截率**乘一次｜资料末尾「勘误」说应按**每艘船的拦截率**单独算 | 多船时不同 | 倾向按勘误。**未动** |\n';
T += '| 9 | **维修的批次结构** | 引擎每帧连续回血（现状）｜资料：维修量 500 / 次数 4 / **持续 32s（每次间隔 8s）/ 冷却 12s / 锁定时间**，且有**命中判定**（会"断奶"） | **44%**（谷神 5769/分 vs 引擎 4000/分） | 倾向按资料改，但只有 10 门有维修值 |\n';
T += '| 10 | **系统伤害效率的语义** | 引擎当成"每发一次性骰子毁系统"（现状）｜资料：**命中分流比率 60% / 40% / 20%**，且系统要有独立血量、累计打满才毁 | **几十倍**（维塔斯A 绝育大帝动力：资料要 6 轮，引擎期望 1.67 发） | 必须按资料改，但**卡在 K1-12 系统血量** |\n';
T += '| 11 | **暴击的「敌方暴击伤害下降」** | 公式里有「基础暴伤×(1+爆伤加成−**敌方暴伤下降**)」｜引擎没实现，**数据里也没这个字段** | 未知 | 要加吗？字段叫什么？ |\n';
T += '| 12 | **轰炸距离** | 命中 ±2%/吉米 **已做**｜**每 1 吉米飞行往复时间 +2 秒 未做** | 未知 | 要不要补 |\n';
T += '| 13 | **武器轻重（轻型/重型）** | 资料：轻型打小型命中更高、重型打大型更好瞄小型大降｜**数据和引擎都没有这个属性** | 未知 | 要的话怎么判？按单发？按名字？ |\n';
T += '| 14 | **「舰队内XX舰船」类加点** | 资料里很多「舰队中所有 X 舰船…」（如 AC721 前三轮冷却缩短、协同集火）｜引擎在 1v1 里当"自己"处理 | 多舰时低估 | 要做成真·舰队级吗 |\n\n';

H(2, 'K3 【我拿不准】我自己没把握的地方 —— 请你验');
T += '> 这一节是**我主动交代**的：下面每一条我"做了但不敢打包票"，或者"没做但说不清该不该做"。\n\n';
T += '| # | 我做了什么 | 我不确定什么 | 怎么验 |\n|---|---|---|---|\n';
T += '| 1 | 条件触发识别了 **' + cond.length + ' 个**节点，引擎每 tick 求值 | 我识别的 6 种条件类型（血量阈值 / 开战前X秒 / 开战后 / 每X秒 / 每X轮 / 前X轮）**是否覆盖全**？占位符和中文数字的解析有没有错？ | 游戏里点开几条对照 |\n';
T += '| 2 | 舰队级机制 **5 类 ' + fleetMechNodes.length + ' 个**节点（多目标反击 / 庇护作战 / 切入作战 / 维修提升 / 命中提升） | 判据「必须是指定旗舰 + 指挥系统未被摧毁」是**按你口述写的**，是否完整？ | 游戏里实测 |\n';
T += '| 3 | 「锁定效率」从命中公式**挪去缩锁定时间** | 你只说"不加命中"，**"缩锁定时间"是我推的** —— 它也可能完全另有含义（比如影响转火速度） | 游戏里看这条加点面板怎么变 |\n';
T += '| 4 | 删掉了普鲁图斯之盾「庇护作战」的硬编码 | 删掉后**完全依赖加点节点**。如果游戏里它是"旗舰自带"而不是"加点给的"，那我就删错了 | 游戏里不点这条加点看还生不生效 |\n';
T += '| 5 | 拦截率只认了 **' + _icpt + ' 处**（光锥M1 / 雷火B2 / 大盾B3 / 太阳鲸C3 / CV3000 A2） | **会不会还有别的船有拦截**，只是资料没写、我没找到？ | 游戏里看其它防空船的面板 |\n';
T += '| 6 | 调校系数 1.3 我**留着没动** | 如果游戏面板其实**已含调校**，那面板口径本身就要重算；如果没含，现在每门白送 30% | 拿一场战斗的实际伤害倒推 |\n';
T += '| 7 | 我报的实现率 **95.7%**（4454 / 4653 战斗相关） | 分母 4653 是**我判的**；判"战斗相关/非战斗"只能用说明文字（`stat` 字段会误判），**可能仍有误判** | 抽查几条 |\n';
T += '| 8 | 分伤用 `round(可打船数 / 2.5)` | 资料是"小数点前主承伤、小数点后副承伤"，我用了四舍五入 —— 可能整体就不对，不只是取整方向 | 见 K2-4 |\n';
T += '| 9 | 攻击序列**只取第一个匹配的列** | 资料说"每一列攻击序列都可以勾选多类型"，我理解成"有序取第一个匹配"。如果实际是**取并集**，选目标逻辑整个要改 | 游戏里看武器面板的攻击序列 |\n';
T += '| 10 | 打系统数据 9 条 | 其中 **4 条相互雷同**（疑我从资料复制错），另外 5 条资料里查不到 | 见 K1-6 |\n';
T += '| 11 | 回归测试 **127 项全绿** | **但这些测试是我自己写的** —— 测试通过 ≠ 和游戏一致，只说明"引擎没被我改坏" | — |\n';
T += '| 12 | 我做了这么多公式校正 | **一次真实对局都没跑过**。所有校正是"资料 → 公式 → 回代资料自己举的数"，**闭环在自己身上**，没有外部真值 | **强烈建议**：你发一场真实战报（配队 + 双方损失 + 时长），我跑 `test/fleet_vs_reality.js` 对比 |\n\n';
...
H(2, '3.1 术语代号（' + (pureRefs.length + embedRefs.length) + ' 个节点 / ' + codeSet.size + ' 种代号）');
T += '**全部代号**：' + [...codeSet].sort().map(c => '`' + c + '`').join(' · ') + '\n\n';
